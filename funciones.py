...
def obtener_vinculacion(correo):
    vinculacion = db['administradores'].find_one({'correo': correo})
    return vinculacion

def obtener_departamento(correo):
    departamento = db['administradores'].find_one({'correo': correo})
    return departamento
...
def eliminar_documento_alumno(id_alumno):
    documentos_collections = ['documentos_TSU', 'documentos_LIC_ING', 'documentos_foraneas', 'documentos_especiales']
    for collection in documentos_collections:
        documento = db[collection].find_one({'id_usuario': id_alumno})
        if documento:
            db[collection].delete_one({'id_usuario': id_alumno})
            return True
    return False
